/*
 * ----------------------------------------------------------------------------
 *  Copyright (c) Manuel Diaz Rojo form WinT 3794. All Rights Reserved.
 *  Open Source Software - may be modified and shared by FRC teams.
 *  This code is under MIT License. Check LICENSE file at project root .
 * ----------------------------------------------------------------------------
 */

package me.macnolo.libds.etc;

public class Utilities {
    public static final int SEND_TIMER = 1;
    public static final int REC_TIMER = 50;

    public static final int ROBOT_PORT = 9999;
    public static final int FMS_PORT = 9999;
    public static final int RADIO_PORT = 9999;
    public static final int DATA_TRANSFER_LENGHT = 4;

    public static final String CREDITS_PROTOCOL = "By Manuel Diaz from WinT 3794.";

    public static final int COLOR_ACTIVE_RED = 0xffcc0000;
    public static final int COLOR_ACTIVE_GREEN = 0x4ca050;

    public static final byte[] DRIVER_STATION_VERSION_2014 = {0x31,0x34,0x30,0x32,0x31,0x37,0x30,0x30};

}
